...
public class KDPointTests {
	...
	public void testKDPointDoubleArray() {
		BigDecimal bd1 = new BigDecimal(2.12), bd2 = new BigDecimal(-9.23),
				bd3 = new BigDecimal(0.01), bd4 = new BigDecimal(-34);
		KDPoint point = new KDPoint(bd1, bd2, bd3, bd4);
		assertEquals(4, point.coords.length);
		assertEquals(bd1, point.coords[0]);
		assertEquals(bd2, point.coords[1]);
		assertEquals(bd3, point.coords[2]);
		assertEquals(bd4, point.coords[3]);
	}
